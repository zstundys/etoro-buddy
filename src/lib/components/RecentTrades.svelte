<script lang="ts">
  ...
  const closedProfit = $derived(
    closedRows.reduce((s, r) => s + (r.pnl ?? 0), 0),
  );
  ...
  {:else}
    <div class="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-5">
      ...
      <div class="rounded-xl border border-border bg-surface-raised px-4 py-3">
        ...
        <p class="text-lg font-semibold">
          {closedRows.length > 0
            ? pctFmt.format((winners / closedRows.length) * 100)
            : "—"}
    ...
    >
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          <thead>
            <tr
              class="border-b border-border text-left text-[10px] font-medium uppercase tracking-wider text-text-secondary"
            >
            ...
            {#each rows as row, i (row.id || i)}
              <tr class="transition-colors hover:bg-surface-overlay/20">
                ...
                <td class="px-3 py-2.5">
                  ...
                  {:else}
                    <span
                      class="inline-flex rounded-full bg-text-secondary/15 px-2 py-0.5 text-[10px] font-medium text-text-secondary"
                    >
                      CLOSED
                    </span>
                  {/if}
                ...
                <td class="px-3 py-2.5">
                  <span
                    class="inline-flex rounded-full px-2 py-0.5 text-[10px] font-medium {row.isBuy
