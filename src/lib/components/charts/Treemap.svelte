<script lang="ts">
	...
	interface LeafData {
		name: string; totalAmount: number; totalPnl: number;
		avgPnlPct: number; count: number;
	}
	interface SectorData {
		name: string; isSector: true; totalAmount: number;
		totalPnl: number; avgPnlPct: number; symbolCount: number;
		children: LeafData[];
	}
	interface RootData { name: string; children: (LeafData | SectorData)[] }
	type NodeData = RootData | SectorData | LeafData;
	type RectNode = d3.HierarchyRectangularNode<NodeData>;

	let { positions, sectorMap = new Map() }: {
	...
	}>({ show: false, x: 0, y: 0, name: '', amount: 0, pnl: 0, pnlPercent: 0, count: 0 });
	...
	$effect(() => {
		...
		const root = d3
			...
			.sum((d) => ('count' in d && !isSector(d)) ? (d as LeafData).totalAmount : 0)
		...
		if (_useSectors) {
			...
			sg.append('rect')
				...
				.attr('fill', (d) => {
					...
					if (c) { c.opacity = 0.1; return c.formatRgb(); }
					return 'transparent';
				})
				.attr('stroke', (d) => {
					const c = d3.color(sectorColorScale(d.data.name));
					if (c) { c.opacity = 0.3; return c.formatRgb(); }
					return 'transparent';
				})
				.attr('stroke-width', 1)
				.on('mouseenter', function (event, d) {
					...
					tooltip = {
						...
						name: data.name, amount: data.totalAmount,
		...
		tile
			...
			.text((d) => {
				const w = d.x1 - d.x0;
				const h = d.y1 - d.y0;
				return w >= MIN_LABEL_W && h >= MIN_AMOUNT_H ? fmt.format((d.data as LeafData).totalAmount) : '';
			});
