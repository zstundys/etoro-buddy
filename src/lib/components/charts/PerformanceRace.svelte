<script lang="ts">
	...
	$effect(() => {
		...
		const visibleSeries = hasSelection ? allSeries.filter((s) => sel.has(s.symbol)) : allSeries;
		...
		const yAxis = d3.axisLeft(yScale).ticks(5).tickFormat((d) => `${Number(d)}%`);
		...
		const allSortedDates = [...new Set(allDates.map((d) => d.getTime()))].sort((a, b) => a - b).map((t) => new Date(t));
		...
		overlay
			.on('mousemove', function (event) {
				...
				const i = Math.min(bisect(allSortedDates, x0), allSortedDates.length - 1);
		...
		>
			...
			{#each tooltip.lines.slice(0, 8) as item (item.symbol)}
				<div class="mt-0.5 flex items-center gap-1.5">
					...
					<span class="{item.pct >= 0 ? 'text-gain' : 'text-loss'}">{item.symbol}: {item.pct >= 0 ? '+' : ''}{pctFmt.format(item.pct)}%</span>
				</div>
			{/each}
		</div>
	{/if}
