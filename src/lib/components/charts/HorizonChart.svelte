<script lang="ts">
  ...
  const chartHeight = $derived.by(() => {
    ...
    const count = groups.filter((g) =>
      candleMap.has(g.positions[0].instrumentId),
    ).length;
  ...
  $effect(() => {
    ...
    const totalHeight =
      strips.length * BAND_HEIGHT + MARGINS.top + MARGINS.bottom;
      ...
      stripG
        ...
        .on("mousemove", function (event) {
          ...
          const bisect = d3.bisector(
            (d: (typeof strip.data)[number]) => d.date,
          ).left;
