<script lang="ts">
	...
	$effect(() => {
		...
		const xScale = d3.scaleTime().domain([months[0], months[months.length - 1]]).range([0, innerWidth]);
		...
		const chartG = svg.append('g').attr('transform', `translate(${MARGINS.left},${MARGINS.top})`);

		if (months.length === 1) {
			// Fallback: simple stacked bars for single month
			const stack = d3.stack<Record<string, number | Date>>().keys(symbols).offset(d3.stackOffsetNone);
				...
				barRect
					.on('mouseenter', function (event) {
						...
						tooltip = {
							...
							monthLabel: d3.timeFormat('%B %Y')((d.data as Record<string, Date>).month as Date)
						};
					})
					.on('mousemove', function (event) {
						const containerRect = containerEl!.getBoundingClientRect();
						tooltip = { ...tooltip, x: event.clientX - containerRect.left, y: event.clientY - containerRect.top };
					})
					.on('mouseleave', () => {
						hoveredSymbol = null;
			...
			const bisect = d3.bisector((d: Record<string, number | Date>) => d.month as Date).left;
		...
		chartG.selectAll('.axis text').attr('fill', COLORS.textSecondary).attr('font-size', 10);
...
<div class="relative flex flex-col">
	...
	{#if tooltip.show}
		...
		>
			...
			<div class="mt-0.5 text-text-secondary">Invested: {fmt.format(tooltip.totalInvested)}</div>
		</div>
	{/if}
</div>
