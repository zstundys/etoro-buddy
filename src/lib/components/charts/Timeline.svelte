<script lang="ts">
	...
	const chartHeight = $derived.by(() => {
		const totalRows = sortedGroups.reduce((sum, g) => sum + g.positions.length, 0);
		const totalGroupGaps = Math.max(0, sortedGroups.length - 1) * GROUP_GAP;
		return Math.max(200, MARGINS.top + MARGINS.bottom + totalRows * ROW_HEIGHT + totalGroupGaps);
	});
	...
	$effect(() => {
		...
		type RowInfo = { group: typeof visible[0]; pos: (typeof allPositions)[0]; y: number };
		const rows: RowInfo[] = [];
		let currentY = 0;
		...
		for (const group of visible) {
			...
			const sorted = [...group.positions].sort(
				(a, b) => new Date(a.openDateTime).getTime() - new Date(b.openDateTime).getTime()
			);
			for (const pos of sorted) {
				rows.push({ group, pos, y: currentY });
				currentY += ROW_HEIGHT;
			}
			groupYRanges.push({ symbol: group.symbol, yStart: groupStart, yEnd: currentY });
			currentY += GROUP_GAP;
		}
