<script lang="ts">
  ...
  $effect(() => {
    ...
    buildLogoColorMap(positions).then((m) => {
      colorMap = m;
    });
  });
...
{#if positions.length > 0}
  <section class="mt-10 mb-10">
    <div class="mb-6 flex items-center gap-3">
      <div
        class="flex h-8 w-8 items-center justify-center rounded-lg bg-brand/15"
      >
        <svg
    ...
    <div class="flex flex-col gap-4">
      ...
      {:else}
        <div class="rounded-xl border border-border bg-surface-raised p-5">
          <h3
            class="mb-3 text-xs font-medium uppercase tracking-wider text-text-secondary"
          >
            Portfolio Value Over Time
          </h3>
          <div
            class="flex items-center justify-center gap-3 py-12 text-sm text-text-secondary"
          >
            <div
      ...
      <ChartCard
        ...
        description="Group instruments into weighted buckets that sum to 100%. The donut chart overlays actual holdings on top of targets — taller slices are overweight, shorter are underweight. Expand a bucket to see per-symbol market values and weights. Below, the Cash Allocation Preview simulates how your available cash (credit minus pending orders) could be deployed across four strategies."
      >
        {#snippet actions()}
          <button
            type="button"
