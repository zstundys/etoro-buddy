<script lang="ts">
	...
	import { currency as fmt, percent as pctFmt, shortDate as dateFmt, normalizeSymbol } from '$lib/format';
	...
	$effect(() => {
		...
		const xDomain = xExtent[0] && xExtent[1] ? xExtent : [new Date(), new Date()];
		const yMin = yExtent[0] !== undefined ? Math.min(yExtent[0], 0) : -10;
		const yMax = yExtent[1] !== undefined ? Math.max(yExtent[1], 0) : 10;
		const yDomain: [number, number] = [yMin, yMax];
		const amountDomain = amountExtent[0] !== undefined && amountExtent[1] !== undefined ? amountExtent : [0, 1];
