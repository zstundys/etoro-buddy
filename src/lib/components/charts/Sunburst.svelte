<script lang="ts">
  ...
  $effect(() => {
    ...
    const ro = new ResizeObserver((e) => {
      if (e[0]) width = e[0].contentRect.width;
    });
  ...
  $effect(() => {
    ...
    const allNodes = (root.descendants() as ArcNode[]).filter(
      (d) => d.depth > 0,
    );
    ...
    const focusNode = _focus
      ? allNodes.find(
          (d) => d.data.name === _focus && d.children && d.children.length > 0,
        )
      : null;
    ...
    g.selectAll<SVGPathElement, ArcNode>("path.arc")
      .data(visibleNodes, (d) => {
        if ("positionId" in d.data)
          return `p-${(d.data as PosNode).positionId}`;
        ...
        if (isSectorNode(data)) {
          ...
          tooltip = {
            ...
            date: pd.openDateTime
              ? dateFmt.format(new Date(pd.openDateTime))
              : undefined,
          };
        }
      })
    ...
    g.selectAll<SVGTextElement, ArcNode>("text.arc-label")
      ...
      .attr("transform", (d) => {
        ...
        const r =
          innerR + ((d.y0 - yOffset + (d.y1 - yOffset)) / 2) * yScaleFactor;
    ...
    if (focusNode) {
      ...
      g.selectAll<SVGTextElement, ArcNode>("text.sub-label")
        .data(
          secondNodes,
          (d) =>
            `sl-${"positionId" in d.data ? (d.data as PosNode).positionId : d.data.name}`,
        )
        .join("text")
      ...
      >
        ...
        {#if tooltip.count}
          <div class="mt-0.5 text-text-secondary">
            {tooltip.count}
            {tooltip.label === "Sector"
              ? "symbol"
              : "position"}{tooltip.count !== 1 ? "s" : ""}
          </div>
        {/if}
        {#if tooltip.date}
