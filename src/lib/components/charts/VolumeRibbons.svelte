<script lang="ts">
  ...
  $effect(() => {
    ...
    const visibleSeries = hasSelection
      ? series.filter((s) => sel.has(s.symbol))
      : series;
    ...
    if (useNormalized) {
      ...
      const visiblePrices = visibleSeries.flatMap((s) =>
        s.data.map((d) => d.close),
      );
    ...
    for (const s of series) {
      ...
      const fillOpacity = !isVisible
        ? 0.03
        : isHoverTarget || currentHover === null
          ? 0.35
          : 0.12;
      ...
      g.append("path")
        ...
        .attr(
          "fill",
          d3.color(color)!.copy({ opacity: fillOpacity }).toString(),
        )
        .attr("d", ribbonArea)
        .style("transition", "fill 0.2s");

      const centerLine = d3
        .line<(typeof s.data)[number]>()
        .x((d) => xScale(d.date))
        .y((d) => yScale(getY(d)))
        .curve(d3.curveMonotoneX);
      ...
      g.append("path")
        ...
        .attr(
          "opacity",
          !isVisible ? 0.08 : currentHover === null || isHoverTarget ? 1 : 0.3,
        )
        .attr("d", centerLine)
    ...
    const yAxis = d3
      ...
      .ticks(5)
      .tickFormat((d) =>
        useNormalized ? `${Number(d)}%` : `$${d3.format(",.0f")(d as number)}`,
      );
      ...
      .on("mousemove", function (event) {
        ...
        for (const s of tooltipSeries) {
          const bisect = d3.bisector(
            (d: (typeof s.data)[number]) => d.date,
          ).left;
          const i = Math.min(bisect(s.data, x0), s.data.length - 1);
          const d = s.data[i];
          const getY = useNormalized
            ? ((d.close - s.openRate) / s.openRate) * 100
            : d.close;
  ...
  {#if tooltip.show}
    ...
    >
      ...
      <div class="mt-0.5 text-text-secondary">
        Price: <Money value={tooltip.price} public />
      </div>
