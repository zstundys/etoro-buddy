<script lang="ts">
	...
	const scales = $derived.by(() => {
		...
		const yScale = d3.scaleLinear().domain([yExtent[0] - pad, yExtent[1] + pad]).range([height - 1, 1]);
	...
	const color = $derived(
		data.length >= 2 && data[data.length - 1].value >= data[0].value ? COLORS.gain : COLORS.loss
	);
...
{#if data.length >= 2}
	...
	{#if hoverPoint}
		...
		>
			...
			<span class="ml-1 {hoverPoint.pct >= 0 ? 'text-gain' : 'text-loss'}">{hoverPoint.pct >= 0 ? '+' : ''}{hoverPoint.pct.toFixed(1)}%</span>
		</div>
	{/if}
{:else}
