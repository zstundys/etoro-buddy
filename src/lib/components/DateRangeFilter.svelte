<script lang="ts">
  ...
  const trackRight = $derived(
    totalDays > 0 ? ((totalDays - endDay) / totalDays) * 100 : 0,
  );
  ...
  function isActiveYear(y: number): boolean {
    ...
    const clampedEndDay =
      expectedEnd > maxDate ? dateToDay(maxDate) : dateToDay(expectedEnd) - 1;
    return (
      dateToDay(startDate) === dateToDay(clampedStart) &&
  ...
  <div class="flex flex-wrap items-center gap-1">
    {#each quickPicks as qp (qp.label)}
      <button
        ...
        class="rounded-md px-2 py-0.5 text-[10px] font-medium transition-colors {isActivePick(
          qp.months,
        )
...
{:else}
  <div
    class="overflow-x-auto rounded-xl border border-border bg-surface-raised px-5 py-4"
  >
    <div class="min-w-fit">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="flex gap-1">
            {#each quickPicks as qp (qp.label)}
              <button
                onclick={() => pickRange(qp.months)}
                class="rounded-md px-2.5 py-1 text-[11px] font-medium transition-colors {isActivePick(
                  qp.months,
                )
                  ? 'bg-brand text-surface'
                  : 'text-text-secondary hover:bg-surface-overlay hover:text-text-primary'}"
      ...
      {#if totalDays > 0}
        <div bind:this={sliderContainer} class="relative mt-3 h-6">
          <div class="absolute inset-0 mx-2">
            <div
              class="pointer-events-none absolute inset-x-0 top-1/2 h-1 -translate-y-1/2 rounded-full bg-surface-overlay"
            ></div>
            {#each yearSegments as seg (seg.year)}
              <button
                onclick={() => pickYear(seg.year)}
                class="absolute top-0 z-5 h-full transition-colors hover:bg-text-secondary/5 {isActiveYear(
                  seg.year,
                )
                  ? 'bg-brand/8'
                  : ''}"
            ...
            {#each yearTicks as tick (tick.year)}
              <div
                class="pointer-events-none absolute top-1/2 z-6 h-3 w-px -translate-y-1/2 bg-text-secondary/30"
                style="left: {tick.pct}%"
              ></div>
            {/each}
            <div
              ...
              class="absolute top-1/2 z-10 h-5 -translate-y-1/2 {dragging
                ? 'cursor-grabbing'
                : 'cursor-grab'}"
              style="left: {trackLeft}%; right: {trackRight}%"
            >
              <div
                class="pointer-events-none absolute inset-x-0 top-1/2 h-1 -translate-y-1/2 rounded-full bg-brand/60"
              ></div>
            </div>
          </div>
          <input
        ...
        <div class="relative mt-1 mx-2 h-4">
          <span class="absolute left-0 text-[10px] text-text-secondary"
            >{dateFmt.format(minDate)}</span
          >
          {#each yearTicks as tick (tick.year)}
            {#if tick.showLabel}
              <span
                class="absolute -translate-x-1/2 text-[10px] {isActiveYear(
                  tick.year,
                )
                  ? 'font-medium text-brand'
                  : 'text-text-secondary/50'}"
                style="left: {tick.pct}%">{tick.year}</span
              >
            {/if}
          {/each}
          <span class="absolute right-0 text-[10px] text-text-secondary"
            >{dateFmt.format(maxDate)}</span
          >
        </div>
      {/if}
    </div>
