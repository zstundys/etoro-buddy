...
function enrichPositions(rawPositions: Position[], instruments: Instrument[], rates: Rate[]): { positions: EnrichedPosition[]; totalInvested: number; totalPnl: number } {
...
export async function fetchPortfolio(keys: ApiKeys): Promise<PortfolioData> {
	...
	const { positions, totalInvested, totalPnl } = enrichPositions(rawPositions, instruments, rates);
	return { positions, credit, totalInvested, totalPnl };
}

export async function fetchTradeHistory(keys: ApiKeys, days: number = 90): Promise<EnrichedTrade[]> {
