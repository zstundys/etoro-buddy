<script lang="ts">
  ...
  let editForm = $state({ symbol: "", buyDate: "", buyPrice: "", units: "", source: "" });

  let addForm = $state({ symbol: "", buyDate: "", buyPrice: "", units: "", source: "" });
    ...
    <div class="mb-4 flex items-baseline justify-between">
      ...
      {#if etoroPositions.length > 0}
        <span class="text-sm text-text-secondary">
          {etoroPositions.length} position{etoroPositions.length !== 1 ? "s" : ""}
          ...
          <table class="w-full text-sm">
            <thead class="sticky top-0 bg-surface-raised">
              <tr class="border-b border-border/50 text-[10px] font-medium uppercase tracking-wider text-text-secondary">
                ...
                <th class="rounded-tr-xl py-2.5 pl-3 pr-4 text-right">Current</th>
              </tr>
            </thead>
            <tbody>
              {#each etoroPositions as pos, i (pos.positionId || i)}
                <tr class="border-b border-border/20 last:border-b-0 transition-colors hover:bg-surface-overlay/20">
    ...
    <div class="mb-4 flex items-baseline justify-between">
      ...
      <div class="flex items-center gap-3">
        {#if manualStore.holdings.length > 0}
          <span class="text-sm text-text-secondary">
            {manualStore.holdings.length} holding{manualStore.holdings.length !== 1 ? "s" : ""}
